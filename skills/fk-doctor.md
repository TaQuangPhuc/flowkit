...
| `PUBLIC_ERROR_MODEL_ACCESS_DENIED` | **Per-account entitlement**, not a tier and not a quota: that Google account was never granted the free low-priority Veo models. Same prompt on another nick submits fine | Nick is parked for **video only** (`MODEL_DENIED_PARK_S`, 6h) and the job re-routes to a nick that does have access | Open the nick's Flow tab and try one video by hand. If the UI refuses too, the account is the problem — delete the nick (`DELETE /api/accounts/{id}`) or get video access granted on it. `/fk-change-model` cannot help: the low-priority models are the only ones the batch path accepts (`require_low_priority_model()`) |
...
| `PUBLIC_ERROR_MODEL_ACCESS_DENIED` on `eb1hJf` / `YhhmEf` | HTTP **200** with an ErrorInfo envelope — the account has no video entitlement. The session and the proxy are both healthy, so do not re-login and do not rotate the proxy | Failover to another nick; that nick parked for video 6h; no auth strike, no unusual strike, no quarantine. Nothing was submitted, so a retry cannot double-render | §C3 `model_access_denied`, below |
...
| `model_access_denied` (503) + `denied_nicks[]` | **Every** connected nick answers `PUBLIC_ERROR_MODEL_ACCESS_DENIED`, so no candidate is left for a video submit. **Nothing was submitted upstream** | `retryable: true`, `retry_after_s` (300 default) + `Retry-After`; `ParkedBackoff` waits it out. A later success on that nick un-parks it and auto-resolves its `MODEL_ACCESS_DENIED` incident | Read `denied_nicks` in the response, or `GET /api/flow/status` → `model_denied` (`{nick: {parked_for_s, until}}`); the `/nicks` page shows a red **no video access** badge. Add a nick whose Google account can render video in the Flow UI, or delete the denied ones. Images, uploads and polls still work on a denied nick |
...
| `PROXY_CONSECUTIVE_ERRORS` incident, proxy quarantined 15m, nick unavailable | `ProxyHealthRecord.consecutive_errors` is a **lifetime** counter persisted to `FATIGUE_STATE_FILE`, so one bad hour used to re-quarantine that proxy on every 60s sweep forever — including the local surfshark bridge `127.0.0.1:18888` and proxies from retired pools | `_sweep_proxies` now calls `decay_stale_errors(PROXY_ERROR_DECAY_S)` (30m): a streak whose `last_error_at` is older than the window is forgotten and its open incident resolved. Account-level errors (`MODEL_ACCESS_DENIED`) never touch the proxy counters | Nothing, if the log says `Forgot stale error streak on …`. A streak that keeps re-arming inside 30m is a real proxy problem — check the gateway |
| A deleted nick still takes jobs / shows in `/health` `workers[]` | `delete_account()` only rewrote `accounts.json`; the nick's Chrome and its WS session stayed alive and stayed routable | `DELETE /api/accounts/{id}` now stops that Chrome (`chrome_stopped` in the response), clears its video park, and `_profile_candidates` skips any session whose account row is gone | If an orphan Chrome predates this fix, kill it: `pkill -f /home/pc/.flowkit/chrome/<nick>` |
...
| `MODEL_ACCESS_DENIED` on video, some nicks only | Per-account, not tier. Those accounts have no video entitlement — the router parks them for video and routes around them. `GET /api/flow/status` → `model_denied`. Delete them or get access granted (§A, §C3) |
| 503 `model_access_denied` on every video call | *All* nicks are denied — the fleet has no video-capable account left. Nothing was submitted; add a nick that can render video in the Flow UI |
